...
import com.itplus.mm.server.workflow.app.AgreeUserListByDevProcessGetApp;
import com.itplus.wf.def.act.WfactModel;

public class ModelProcessMailNotify  implements com.itplus.wf.inst.app.IApplication{
 	public String execute(DBAssistant dba, WSParam params, WfactModel model) throws Exception {
 		 
		String title = SysHandler.getInstance().getProperty("MAIL_MODEL_TITLE");	// 선택된 협의자에게 보내는 메일 제목
		String title2 = SysHandler.getInstance().getProperty("MAIL_MODEL_APPROVER_TITLE");	// 선택되지 않은 협의자에게 보내는 메일 제목
		
		AgreeUserListByDevProcessGetApp agreeUsers = new AgreeUserListByDevProcessGetApp();	// 전체 협의자 목록
